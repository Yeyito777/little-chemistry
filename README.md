...
Generated behavior is server-side Java loaded in a dedicated class loader for each definition. The initial behavior API exposes separate callbacks for right-clicking in air and right-clicking a block, with direct access to `ServerLevel`, `ServerPlayer`, the mutable stack, hit position/face, and the definition. Returning `PASS` preserves ordinary placement, food consumption, armor equipping, or block-item behavior; handled callbacks can return Minecraft's native interaction results. Compilation is transactional, persisted source is recompiled when the server loads, a failed replacement never enters the catalog, and a behavior that throws while running is disabled. This experimental mode intentionally executes AI-authored code with the server process's authority and should only be enabled where the server operator trusts generated code.
