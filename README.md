...
Every definition has a server-side Java class loaded in its own class loader. The behavior API exposes callbacks for item and placed-block events with direct access to the relevant server-side Minecraft objects and the definition. The interface has no inherited callback implementations: each generated class must implement the complete contract, using `PASS`, no-op bodies, or the unchanged result explicitly where it wants ordinary carrier behavior. Compilation is transactional, persisted source is recompiled when the server loads, a failed replacement never enters the catalog, and a behavior that throws while running is disabled. Catalogs from older versions are backed up and upgraded by adding explicit neutral methods to legacy classes that previously relied on interface defaults. This experimental system executes AI-authored code with the server process's authority and should only be enabled where the server operator trusts generated code.
