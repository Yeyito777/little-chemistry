...
Creation runs asynchronously through a fast-service `gpt-5.6-sol` content compiler using medium reasoning. Sol receives the requested name and kind as design data and can only mutate an isolated draft through narrowly scoped tools. Its vanilla-content fetch tool returns set-texture-compatible artwork plus relevant gameplay metadata for similar items and blocks, including tool behavior, reach, durability, food/equipment traits, hardness, light, collision, redstone traits, and mining requirements. Sol supplies an indexed 16×16 texture, bounded block or item properties, optional block particles, and block light behavior, then calls `submit`. Java validation is authoritative; incomplete drafts never enter the world. The agent has no fixed round, tool-call, invalid-submit, or overall generation timeout; server shutdown still cancels active work.
...
The production JAR is written to `build/libs/little-chemistry-1.1.0.jar`.
